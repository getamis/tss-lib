...
func TestScalarLinearCombination(t *testing.T) {
	curveList := []*elliptic.CurveParams{elliptic.P224().Params(), elliptic.P256().Params(), elliptic.P384().Params()}

	for i := 0; i < len(curveList); i++ {
		pointList := make([]*ECPoint, 4)
		...
		scalarList[2] = big.NewInt(-5)
		scalarList[3] = big.NewInt(2)
		scalarList[3].Mod(scalarList[3], curveList[i].N)

		result, err := ComputeLinearCombinationPoint(scalarList, pointList, curveList[i])

		expect := ScalarBaseMult(curveList[i], big.NewInt(2))

		if err != nil || result.X().Cmp(expect.X()) != 0 || result.Y().Cmp(expect.Y()) != 0 {
			t.Errorf("Add() expect = nil,nil, got X = %v, Y=%v", result.X(), result.Y())
		}
	}
}
